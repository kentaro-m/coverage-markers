...
import { findLcovFilePath, readLcovFile, parseLcovInfo, getLcovInfoForActiveEditor, renderMarkers, deleteMarkers } from './lcov';
  ...
  subscriptions.add(
    atom.workspace.observeTextEditors(async (editor) => {
      try {
        ...
        const targetLcovInfo = getLcovInfoForActiveEditor(editor.getPath(), records);
        renderMarkers(editor, targetLcovInfo);
