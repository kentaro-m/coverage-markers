...
export async function getFileList(projectPath) {
  try {
    ...
    const fileList = await Promise.all(dirList.map(async (file) => {
...
export async function findLcovFilePath() {
  try {
    const filePaths = atom.project.getPaths();

    const fileList = await getFileList(filePaths[0]);
