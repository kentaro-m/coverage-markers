...
function renderCoverage(editor, targetLcovInfo) {
  return Promise.coroutine(function *() {
...
export function displayCoverage(editor) {
  return Promise.coroutine(function *() {
    ...
    for (const dirName of files) {
      ...
      if (fs.statSync(dirPath).isDirectory()) {
        lcovPath = yield getLcovPath(dirPath);
      }
      if (lcovPath) {
        break;
      }
    ...
    if (!lcovPath) {
      yield Promise.reject(new NotFoundLcovFileError('lcov file could not be found.'));
    }
...
export function removeCoverage(editors) {
  if (lcovWatcher) {
    lcovWatcher.unwatch();
  }
